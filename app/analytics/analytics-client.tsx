...
import { QrCodeRecord } from '@/app/actions/qr';
import {
  QrCode as QrIcon,
  Users,
  Clock,
...
const deviceData = [
  { name: 'Mobile', value: 0, color: '#4f46e5' },
  { name: 'Desktop', value: 0, color: '#6cf8bb' },
];

const locationData: { city: string, scans: number }[] = [];

const ranges = ['Last 7 Days', '30 Days', '90 Days'] as const;

export default function AnalyticsClient({ initialQrCodes }: { initialQrCodes: QrCodeRecord[] }) {
  ...
  const codeName = selectedCode?.name || 'All Codes';
